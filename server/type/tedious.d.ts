 ...
 * so I cannot list the values here as enum values
    ...
    7_4 = 0x74000004
    */
declare var TediousTDS_VERSION: { [index: string]: number };

type TediousTediousType = {
    type: string;
    name: string;
}

type TediousTediousTypes = {
    BigInt: TediousType;
    Binary: TediousType;
    Bit: TediousType;
    BitN: TediousType;
...
declare var TYPES: TediousTypes;

type TediousConnectionOptions = {
...
type TediousConnectionConfig = {
     ...
     * Further options
     */
    options?: TediousConnectionOptions;
}

type TediousParameterOptions = {
...
type TediousRequest = {
     ...
     */
    _constructor(sql: string, callback: (error: Error, rowCount: number, rows: any[]) => void);
...
} & TediousEventable<'columnMetadata'|'row'|'done'|'doneInProc'|'doneProc'|'returnValue'>;

type TediousBulkLoadColumnOpts  = {
...
type TediousConnection = {
    ...
    _constructor(config: ConnectionConfig);
     ...
     */
    beginTransaction(callback: (error?: Error) => void, name?: string, isolationLevel?: number): void;
     ...
     * @param isolationLevel The isolation level that the transaction is to be run with.
     */
    transaction(callback: (error: Error, done: (error?: Error) => void) => void, name?: string, isolationLevel?: number): void;
...
} & TediousEventable<'connect'|'end'|'error'|'debug'|'infoMessage'|'errorMessage'|'databaseChange'|'languageChange'|'charsetChange'|'secure'>;


type TediousEventable<T> = {
    on(eventName: T, callback: Function): void;
}